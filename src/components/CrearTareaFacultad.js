...
const CrearTareaFacultad = () => {
  ...
  const tareasFacultadCollection = collection(db, "tareasfacultad");

  const store = async (e) => {
    e.preventDefault();
    
    if (user) {
      await addDoc(tareasFacultadCollection, {
        titulo: titulo,
  ...
  return (
    <div className='container'>
      <div className='row'>
        <div className='col'>
          <h1>Crear tarea para la facultad</h1>
          <form onSubmit={store}>
            <div className='mb-3'>
...
export default CrearTareaFacultad;
